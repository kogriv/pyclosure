...
class MyLogger(logging.Logger):
    """
    A class for creating a custom logger.
    Implementation of the logging method with a given level.
    The function of searching for all existing loggers in the scope
    """
...
class MyLoggerSuper(logging.Logger):
    def __init__(self, name, create_level='INFO', enable_logging=True):
        ...
        # If there are no handlers, add a default handler
        if not self.handlers:
            console_handler = logging.StreamHandler()
            self.addHandler(console_handler)

        # Output information about the current level of each handler
        for handler in self.handlers:
            print(f"Handler: {handler}, Current Level: {handler.level}")

        # Setting the logging level for all handlers
        for handler in self.handlers:
...
def _main(standard_logger_level = 'DEBUG',
    ...
    msg_levels = """
    logging.DEBUG       corresponds to the number 10.
    logging.INFO        corresponds to the number 20.
    logging.WARNING     corresponds to the number 30.
    logging.ERROR       corresponds to the number 40.
    logging.CRITICAL    corresponds to the number 50
    """
    print(f'|||||||||||| Set the basic logging level \
          {standard_logger_level} ||||||||||||||||')
    logging.basicConfig(level= standard_logger_level)
    print(msg_levels)
    print('|||||||||||| Lets create a standard logger without a name |||||||||||||')
    ...
    logger.info('*****************************************************')
    logger.info('*****************************************************')
    logger.info('Creating an instance of MyLogger with the specified logging level and state')
    ...
    # Output the contents of loggerDict after creating loggers
    # print("loggerDict after creating loggers:")
    # print(logging.Logger.manager.loggerDict)

    logger.info('*****************************************************')
    logger.info('We use the mylev method for logging')
